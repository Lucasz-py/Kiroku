...
const CustomDropdown = ({ label, value, options, onChange, disabled = false, placeholder }: CustomDropdownProps) => {
  ...
  return (
    <div className="flex flex-col gap-2 relative" ref={dropdownRef}>
      ...
      <button type="button" disabled={disabled} onClick={() => setIsOpen(!isOpen)} className={`w-full bg-[#0D0F15] text-white border border-zinc-800 p-3.5 flex justify-between items-center transition-colors text-sm font-bold rounded-lg ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:border-[#FF3B3B]/50'}`}>
        <span className="truncate">{selectedOption ? selectedOption.label : placeholder}</span>
        <span className={`text-zinc-500 transition-transform duration-300 ${isOpen ? 'rotate-180 text-[#FF3B3B]' : ''}`}>▼</span>
      </button>
      {isOpen && !disabled && (
        <div className="absolute top-[75px] left-0 w-full bg-[#0D0F15] border border-[#FF3B3B]/50 shadow-[0_0_20px_rgba(255,59,59,0.2)] z-50 max-h-60 overflow-y-auto rounded-lg">
          <button type="button" onClick={() => { onChange(''); setIsOpen(false); }} className={`w-full text-left px-4 py-3 text-xs font-bold transition-colors hover:bg-[#11131A] border-b border-zinc-800/50 ${value === '' ? 'text-[#FF3B3B] bg-[#11131A]/80' : 'text-zinc-400'}`}>{placeholder}</button>
          {options.map((opt) => (
            <button key={opt.value} type="button" onClick={() => { onChange(opt.value); setIsOpen(false); }} className={`w-full text-left px-4 py-3 text-xs font-bold transition-colors hover:bg-[#11131A] border-b border-zinc-800/50 last:border-0 ${value === opt.value ? 'text-[#FF3B3B] bg-[#11131A]/80' : 'text-zinc-400'}`}>{opt.label}</button>
          ))}
        </div>
      )}
    </div>
...
export const Search = () => {
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 pt-32 md:pt-36 font-sans max-w-[1400px] min-h-screen">
      ...
      <div className={`max-w-4xl mx-auto transition-all duration-500 ${isDiscoverMode ? 'mt-8 mb-16' : 'mt-0 mb-6'}`}>
        ...
        <form onSubmit={handleSubmit} className="relative z-20">
          <div className="flex flex-col md:flex-row gap-3">
            <input
              ...
              className="flex-1 p-4 pl-6 text-white bg-[#11131A] border border-[#FF3B3B]/20 focus:border-[#FF3B3B] focus:outline-none focus:ring-1 focus:ring-[#FF3B3B]/50 transition-all font-bold text-sm rounded-lg placeholder:text-zinc-600"
            />
            <div className="flex gap-2 shrink-0">
              <button type="button" title="Filtros Avanzados" onClick={() => setShowFilters(!showFilters)} className={`px-6 font-bold transition-all border flex items-center justify-center rounded-lg ${showFilters ? 'bg-[#FF3B3B]/20 border-[#FF3B3B] text-[#FF3B3B]' : 'bg-[#11131A] border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-600'}`}>
                <Filter size={20} />
              </button>
              <button type="submit" className="bg-[#FF3B3B] text-white px-8 py-4 font-black uppercase tracking-widest hover:bg-[#FF5555] transition-colors text-sm rounded-lg">
                Buscar
              </button>
            </div>
          </div>
          ...
          {instantResults.length > 0 && !showFilters && (
            <div className="absolute top-full left-0 w-full md:w-[calc(100%-180px)] bg-[#11131A] mt-2 border border-zinc-800 shadow-[0_0_30px_rgba(0,0,0,0.8)] z-50 overflow-hidden rounded-lg">
              {instantResults.map((anime) => (
                <Link key={anime.mal_id} to={`/anime/${anime.mal_id}`} className="flex items-center gap-4 p-3 border-b border-zinc-800/50 hover:bg-[#1A1C24] transition-colors last:border-0">
                  <div className="w-12 h-16 bg-[#0D0F15] shrink-0 overflow-hidden rounded-md">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zinc-400 font-bold mt-1">{anime.episodes ? `${anime.episodes} episodios` : 'En emisión'}</p>
                  </div>
                </Link>
              ))}
            </div>
          ...
          {showFilters && (
            <div className="mt-4 bg-[#11131A]/90 backdrop-blur-xl border border-[#FF3B3B]/30 p-6 md:p-8 shadow-[0_0_30px_rgba(255,59,59,0.1)] animate-in fade-in rounded-xl">
              <div className="flex justify-between items-center mb-6 border-b border-zinc-800 pb-4">
                <h3 className="text-sm font-bold text-white uppercase tracking-widest flex items-center gap-2"><Filter size={16} className="text-[#FF3B3B]"/> Filtros de Búsqueda</h3>
                <button type="button" onClick={() => setShowFilters(false)} className="text-zinc-500 hover:text-white transition-colors bg-[#0D0F15] p-2 border border-zinc-800 hover:border-zinc-600 rounded-lg"><X size={16} /></button>
              </div>
              ...
              <div className="mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {GENRES.map(genre => (
                    <button key={genre.id} type="button" onClick={() => toggleGenre(genre.id)} className={`px-4 py-2 text-xs font-bold transition-all border rounded-lg ${localFilters.genres.includes(genre.id.toString()) ? 'bg-[#FF3B3B]/10 border-[#FF3B3B] text-[#FF3B3B] shadow-[0_0_10px_rgba(255,59,59,0.15)]' : 'bg-[#0D0F15] border-zinc-800 text-zinc-500 hover:border-zinc-600 hover:text-white'}`}>{genre.name}</button>
                  ))}
                </div>
              </div>

              <div>
                <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest block mb-3">Estudios Populares</label>
                <div className="flex flex-wrap gap-2">
                  <button type="button" onClick={() => setLocalFilters({...localFilters, studioId: '', studioName: ''})} className={`px-4 py-2 border text-xs font-bold transition-colors rounded-lg ${!localFilters.studioId ? 'bg-[#FF3B3B]/10 border-[#FF3B3B] text-[#FF3B3B]' : 'bg-[#0D0F15] border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-600'}`}>Todos</button>
                  {TOP_STUDIOS.map(studio => (
                    <button key={studio.value} type="button" onClick={() => setLocalFilters({...localFilters, studioId: studio.value, studioName: studio.label})} className={`px-4 py-2 border text-xs font-bold transition-colors rounded-lg ${localFilters.studioId === studio.value ? 'bg-[#FF3B3B]/10 border-[#FF3B3B] text-[#FF3B3B] shadow-[0_0_10px_rgba(255,59,59,0.15)]' : 'bg-[#0D0F15] border-zinc-800 text-zinc-500 hover:bg-[#11131A] hover:text-white hover:border-zinc-600'}`}>{studio.label}</button>
                  ))}
                </div>
              </div>
            </div>
        ...
        {!isDiscoverMode && activeTags.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 mt-4 animate-in fade-in">
            <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest mr-2">Filtros Activos:</span>
            {activeTags.map((tag, idx) => (
              <span key={idx} className="bg-[#FF3B3B]/10 text-[#FF3B3B] border border-[#FF3B3B]/20 text-[10px] md:text-xs font-bold px-3 py-1.5 rounded-full">
                {tag}
              </span>
            ))}
          </div>
      ...
      {loading && (
        <div className="flex flex-col justify-center items-center py-32 text-[#FF3B3B] gap-4">
      ...
      {!loading && results.length > 0 && (
        <div className="animate-in fade-in duration-500">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 border-b border-zinc-800 pb-4 gap-4 mt-8">
            <h3 className="text-xl md:text-2xl font-bold text-white flex items-center gap-3">
              Resultados
              <span className="bg-[#11131A] text-zinc-400 border border-zinc-800 text-xs px-3 py-1 rounded-lg">{results.length} coincidencias</span>
            </h3>
            <button onClick={clearFilters} className="flex items-center gap-2 px-4 py-2 bg-[#11131A] border border-zinc-800 text-zinc-500 hover:text-[#FF3B3B] hover:border-[#FF3B3B]/30 transition-all text-xs font-bold uppercase tracking-widest rounded-lg">
          ...
          {hasNextPage && (
            <div className="flex justify-center mt-16 mb-8">
              <button onClick={() => executeAdvancedSearch(searchParams, page + 1)} disabled={loadingMore} className="flex items-center gap-3 px-8 py-4 bg-[#11131A] border border-zinc-700 hover:border-[#FF3B3B] text-zinc-300 hover:text-[#FF3B3B] font-bold uppercase tracking-[0.2em] text-xs transition-all disabled:opacity-50 rounded-lg">
                {loadingMore ? <><Loader2 size={18} className="animate-spin text-[#FF3B3B]" /> Cargando...</> : <>Cargar más resultados <Plus size={16} className="text-[#FF3B3B]"/></>}
              </button>
            </div>
          )}
        </div>
      ...
      {!loading && results.length === 0 && hasActiveFilters && (
        <div className="text-center py-20 bg-[#11131A]/50 border border-zinc-800 animate-in fade-in rounded-xl">
          <FilterX size={48} className="mx-auto text-zinc-600 mb-4" />
          <p className='text-zinc-300 text-xl font-bold mb-2'>No encontramos animes con esos parámetros exactos.</p>
          <p className='text-zinc-500'>Ajusta los filtros para ampliar la búsqueda.</p>
          <button onClick={clearFilters} className="inline-block mt-6 px-8 py-3 border border-zinc-700 text-zinc-300 hover:bg-[#11131A] hover:border-[#FF3B3B]/30 hover:text-[#FF3B3B] transition-colors rounded-lg">
            Reiniciar Filtros
          </button>
        </div>
      )}
      ...
      {isDiscoverMode && (
        <div className="flex flex-col gap-12 animate-in fade-in duration-700">
          <section className="bg-[#11131A]/80 p-8 border-l-2 border-[#FF3B3B] shadow-[0_0_30px_rgba(0,0,0,0.5)] relative overflow-hidden rounded-xl">
            <div className="flex flex-col md:flex-row items-center justify-center gap-4 lg:gap-50 relative z-10 max-w-5xl mx-auto py-4">
              <div className="text-center md:text-left max-w-md">
                ...
                <p className="text-zinc-400 mb-8 text-lg leading-relaxed">Deja que el destino elija tu próxima aventura. Nuestra base de datos elegirá una serie o película completamente al azar para ti..</p>
                <button onClick={handlePickRandomAnime} disabled={loadingRandom} className="bg-[#FF3B3B] text-white px-8 py-4 font-black tracking-widest text-xs uppercase hover:bg-[#FF5555] transition-colors flex items-center gap-3 mx-auto md:mx-0 disabled:opacity-70 rounded-lg">
                  {loadingRandom ? <Loader2 size={18} className="animate-spin" /> : <Dices size={18} />} {loadingRandom ? 'Calculando...' : 'Generar Anime al Azar'}
                </button>
              </div>
              <div className="w-full md:w-64 min-h-[350px] flex items-center justify-center bg-[#0D0F15] border border-[#FF3B3B]/20 shrink-0 rounded-xl overflow-hidden">
                {randomAnime ? <div className="w-full h-full p-2"><AnimeCard anime={randomAnime} /></div> : <div className="text-zinc-600 flex flex-col items-center gap-3"><Dices size={36} className="opacity-50" /><span className="text-sm font-medium">Esperando inicialización</span></div>}
              </div>
            </div>
          </section>

          <section>
            <div className="flex flex-col sm:flex-row justify-between items-center mb-8 border-b border-zinc-800 pb-4 gap-4">
              <h3 className="text-2xl font-bold text-white flex items-center gap-3">
                Recomendados para ti
              </h3>
              <button onClick={handleLoadRecommendations} disabled={loadingRecs} className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-zinc-500 hover:text-[#FF3B3B] bg-[#11131A] px-4 py-2 border border-zinc-800 hover:border-[#FF3B3B]/50 transition-all disabled:opacity-50 rounded-lg">
                <RefreshCw size={14} className={loadingRecs ? "animate-spin" : ""} /> Actualizar
              </button>
            </div>
            {loadingRecs ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-5">{[...Array(6)].map((_, i) => <div key={i} className="bg-[#11131A] aspect-[3/4] border border-zinc-800 animate-pulse rounded-xl"></div>)}</div>
            ) : (
